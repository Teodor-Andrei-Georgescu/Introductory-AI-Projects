...
for c in classes:
    ...
    likelihoods[c] = class_data["cap-shape"].value_counts(normalize=True).to_dict()
    
    for cap_shape in cap_shapes:
        if cap_shape not in likelihoods[c]:
...
for a in cap_shapes:     
    posteriors[a] = {} 
    for c in classes:
        # Bayes Rule is Posterior = (Likelihood * Prior) / Marginal
        likelihood = likelihoods[c].get(a) 
        prior = priors[c]
