...
nb_clf_cap = CategoricalNB()
nb_clf_cap.fit(X_train_cap, y_train)
y_pred_cap = nb_clf_cap.predict(X_test_cap)
...
# Decision Tree Classifier for the full dataset
...
# Decision Tree Classifier for the `cap-shape` 
dt_clf_cap = tree.DecisionTreeClassifier(max_depth=2, random_state=0)
dt_clf_cap.fit(X_train_cap, y_train)
y_pred_dt_cap = dt_clf_cap.predict(X_test_cap) 
